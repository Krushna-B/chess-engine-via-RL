...
/**
Ad dirichelet noise to priors so can explore routes model may not inital see,
*/
void add_root_dirichlet_noise(Node &root, float alpha, float epsilon,
                              std::mt19937_64 &rng) {
  if (root.children.empty()) {
    throw std::runtime_error("Cannot add root noise before root expansion");
  }
  ...
  /*
   * A Dirichlet sample can be generated by
   * x_i ~ Gamma(alpha, 1)
   * eta_i = x_i / sum(x)
   */
  ...
  if (!std::isfinite(noise_sum) || noise_sum <= 0.0) {
    throw std::runtime_error("Failed to generate Dirichlet noise");
  }

  double final_prior_sum = 0.0;

  for (std::size_t i = 0; i < root.children.size(); ++i) {
    float normalized_noise = static_cast<float>(noise[i] / noise_sum);

    float original_prior = root.children[i]->prior;

    float noisy_prior =
        (1.0f - epsilon) * original_prior + epsilon * normalized_noise;

    root.children[i]->prior = noisy_prior;
    final_prior_sum += noisy_prior;
  }

  if (std::abs(final_prior_sum - 1.0) > 1e-4) {
    throw std::runtime_error("Noisy root priors do not sum to one");
  }
}
...
void validate_policy_target(const PolicyArray &policy) {
  ...
  if (std::abs(sum - 1.0f) > 0.0001f) {
    throw std::runtime_error(
        "Policy probabilities do not sum to one (sum=" + std::to_string(sum) +
        ", nonzero=" + std::to_string(nonzero_actions) + ")");
  }
...
void run_self_play_search(Node &root, NeuralNetwork &network, int simulations,
  ...
  // The first simulation creates the root children
  if (!root.expanded) {
    monte_carlo_tree_sim(root, network);
  ...
  // Add noise exactly once for this real move
  constexpr float DIRICHLET_ALPHA = 0.3f;
  constexpr float DIRICHLET_EPSILON = 0.25f;

  add_root_dirichlet_noise(root, DIRICHLET_ALPHA, DIRICHLET_EPSILON, rng);

  // Complete the requested number of simulations
  for (int i = completed_simulations; i < simulations; ++i) {
    monte_carlo_tree_sim(root, network);
  }
}
